...
최근에 코드리뷰를 하다 accesstoken을 관리하는 로직를 보게되었는데, 클라이언트 컴포넌트가 아닌 클라/서버 혼용이 가능한 유틸함수에서 getState로 직접 꺼내쓰는 패턴이 조금 낯설어 주의깊게 보았던 기억이 있다. 그러나 이내, fallback처리가 안전하게 되어있는듯 하여 클라이언트, 서버에서 모두 정상적으로 동작할것이라 어림짐작 하게 되었다. 하지만 이 코드는 얼마뒤  Next.js 운영 환경에서 **A 유저가 로그인했는데 B 유저의 화면에 A의 계정 정보가 노출되는** 호달달한 버그를 일으켰다.
...
### Q. 그러면 Zustand 스토어는 서버에서 어떻게 존재할까?
...
**왜 모듈을 import할 때마다 새로 실행하지 않는 걸까?**
...
**왜 로컬 개발 환경에서는 이 버그가 잘 안 보이는 걸까?**
로컬에서는 혼자 테스트하기 때문에 타이밍이 겹칠 일이 없다. 트래픽이 높은 운영 환경에서만 드러날 수밖에 없다.
...
### `createStore` + Provider 패턴을 제안하는 이유

결론적으로 문제의 원인은 스토어가 **모듈 수준의 싱글톤**이라는 것이었다. 그렇다면 해결책은 **스토어를 싱글톤으로 만들지 않으면 된다.**
...
### Q. `create()`와 `createStore()`는 무엇이 다를까?
...
### Q. `createStore`로 팩토리 함수를 만드는 것만으로 충분할까?
...
### Q. Provider 코드에서 `useRef`는 왜 필요할까?
...
### Q. 왜 `layout.tsx`에서 쿠키를 읽을까? 쿠키도 공유되지 않을까?
...
### Q. 그러면 서버 함수에서 토큰이 필요할 때는 무조건 쿠키에서 읽어야 할까?
...
### Q. Provider 패턴을 쓰면 Hydration 문제는 없을까?
