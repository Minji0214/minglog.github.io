...
### TBT 변화: Dynamic Import의 trade-off

Dynamic Import를 적용하고 나서 TBT(Total Blocking Time)를 확인해봤더니, **원래 0ms였던 TBT가 약간 올라갔습니다**.

번들을 줄였는데 블로킹이 늘어나는 게 직관적으로 이해가 잘 안 됐는데, 원인도 **타이밍**이었습니다.

- **최적화 전**: 121KB 번들이 페이지 로드 시 한 번에 파싱됩니다. FCP 이전에 모든 JS 실행이 끝나기 때문에 FCP~TTI 구간에 메인 스레드 블로킹이 없었고, TBT = 0이었습니다.
- **최적화 후**: Dynamic Import로 분리한 청크들이 **FCP 이후**에 lazy load됩니다. 이 청크들이 FCP~TTI 사이에 파싱·실행되면서 메인 스레드를 짧게 블로킹하고, 이게 TBT에 반영됩니다.

TBT는 FCP와 TTI 사이의 long task(50ms 초과) 시간의 합입니다. 원래는 그 구간에 블로킹이 없었는데, 청크 로딩 타이밍이 그 구간 안으로 들어온 셈입니다.

LCP는 개선됐고 TBT 증가폭이 크지 않아서 trade-off를 감수했지만, "번들을 줄이면 무조건 좋다"가 아니라 **어느 시점에 실행되느냐**가 중요하다는 걸 직접 확인했습니다.
...
| JS 청크 수 | — | 56개 감소 |
| TBT | 0ms | 약간 증가 (Dynamic Import trade-off) |
